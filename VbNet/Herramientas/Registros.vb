...
Public Class Registros
        ...
        With Datos
            ...
            For n = 0 To .Rows.Count - 1
                If BreakFieldIsChar Then
                    If CEntero(AgrupacionDeRegistros.Compute("count(" & CampoCorte & ")", CampoCorte & "='" & Datos.Rows(n)(CampoCorte).ToString.Replace("'", "''") & "'").ToString) = 0 Then
                        ArgsClone = .Rows(n).ItemArray()
                        ArgsClone(Posicion) = CDoble(Datos.Compute("sum(" & CampoAgrupacion & ")", CampoCorte & "='" & Datos.Rows(n)(CampoCorte).ToString.Replace("'", "''") & "'").ToString)
                        If CampoAgrupacion2.Trim.Length <> 0 Then
                            ArgsClone(Posicion2) = CDoble(Datos.Compute("sum(" & CampoAgrupacion2 & ")", CampoCorte & "='" & Datos.Rows(n)(CampoCorte).ToString.Replace("'", "''") & "'").ToString)
                        End If
                        If CampoAgrupacion3.Trim.Length <> 0 Then
                            ArgsClone(Posicion3) = CDoble(Datos.Compute("sum(" & CampoAgrupacion3 & ")", CampoCorte & "='" & Datos.Rows(n)(CampoCorte).ToString.Replace("'", "''") & "'").ToString)
                        End If
                        AgrupacionDeRegistros.Rows.Add(ArgsClone)
                    End If
                Else
                    If CEntero(AgrupacionDeRegistros.Compute("count(" & CampoCorte & ")", CampoCorte & "=" & Datos.Rows(n)(CampoCorte).ToString).ToString) = 0 Then
                        ArgsClone = .Rows(n).ItemArray()
                        ArgsClone(Posicion) = CDoble(Datos.Compute("sum(" & CampoAgrupacion & ")", CampoCorte & "=" & Datos.Rows(n)(CampoCorte).ToString).ToString)
                        If CampoAgrupacion2.Trim.Length <> 0 Then
                            ArgsClone(Posicion2) = CDoble(Datos.Compute("sum(" & CampoAgrupacion2 & ")", CampoCorte & "=" & Datos.Rows(n)(CampoCorte).ToString).ToString)
                        End If
                        If CampoAgrupacion3.Trim.Length <> 0 Then
                            ArgsClone(Posicion3) = CDoble(Datos.Compute("sum(" & CampoAgrupacion3 & ")", CampoCorte & "=" & Datos.Rows(n)(CampoCorte).ToString).ToString)
                        End If
                        AgrupacionDeRegistros.Rows.Add(ArgsClone)
                    End If
                End If
    ...
    Public Function ExportarExcelXml(ByVal Titulo As String, ByVal Tabla As String, ByVal Request As System.Web.HttpRequest) As String
        ...
        MiFile.Write(Cadena.ToString.Replace("<img src='../images/espacio.gif' style='width:10px;'>", ""))
        MiFile.Close()
